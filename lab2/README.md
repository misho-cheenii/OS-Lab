...
the input to a command from a file.<br/>For example, if a user enters **ls > out.txt** the output from the ls
command will be redirected to the file out.txt. Similarly, input can be redirected as well. For example, if the user enters
**sort < in.txt** the file in.txt will serve as input to the sort command.
